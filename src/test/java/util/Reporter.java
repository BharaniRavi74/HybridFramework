...
public abstract class Reporter  {
	...
	public synchronized void startReport() {
		...
		reporter.config().setDocumentTitle("Salesforce");
	...
	public void reportStep(String desc, String status, boolean bSnap) throws IOException {
		synchronized (test) {
			...
			if (bSnap && !(status.equalsIgnoreCase("INFO") || status.equalsIgnoreCase("skipped")
				...
				byte[] snapNumber ;
				snapNumber = takeSnap();
				img =MediaEntityBuilder.createScreenCaptureFromPath("../report/images/failed.png").build();
			}
